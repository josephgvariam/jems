...
public class JemsInvoiceServiceImpl implements JemsInvoiceService {
	...
	public List<JemsInvoice> findJemsInvoiceEntries(int firstResult, int maxResults) {
        return jemsInvoiceRepository.findAll(new org.springframework.data.domain.PageRequest(firstResult / maxResults, maxResults)).getContent();
    }
...
//        jemsInvoice.setJemsEvent(je);
	...
	public synchronized void saveNewJemsInvoice(JemsInvoice jemsInvoice) {
        ...
        jemsInvoice.setINumber(jemsCountersService.nextInvoiceNumber(jemsInvoice.getJemsEvent().getOrganization()));
