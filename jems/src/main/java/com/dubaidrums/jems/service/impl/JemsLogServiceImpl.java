...
public class JemsLogServiceImpl implements JemsLogService {
	...
	public List<JemsLog> findJemsLogEntries(int firstResult, int maxResults) {
        return jemsLogRepository.findAll(new org.springframework.data.domain.PageRequest(firstResult / maxResults, maxResults)).getContent();
    }
	...
	public List<JemsLog> findLatestLogs(int size) {
		return jemsLogRepository.findAll(new org.springframework.data.domain.PageRequest(0, size, new Sort(Sort.Direction.DESC,"logDateTime"))).getContent();
	}
