...
public class JemsSearchServiceImpl implements JemsSearchService {
	...
	public void setupSearch() {
		String url = "http://localhost:8983/solr/jems";
		try {
	...
	private SolrInputDocument getSolrDocument(JemsEvent je) {
		...
		doc.addField("hiringAgentContactPerson",
				je.getHiringAgentContactPerson());
		...
		// doc.addField( "qNumber",
		// je.getJemsQuotation().getQNumber().toString());
		// doc.addField( "qDate", je.getJemsQuotation().getQDate());
		// doc.addField( "qDescription1",
		// je.getJemsQuotation().getDescription1());
		// doc.addField( "qDescription2",
		// je.getJemsQuotation().getDescription2());
		// doc.addField( "qDescription3",
		// je.getJemsQuotation().getDescription3());
		// doc.addField( "qDescription4",
		// je.getJemsQuotation().getDescription4());
		// doc.addField( "qDescription5",
		// je.getJemsQuotation().getDescription5());
		// doc.addField( "qBillTo", je.getJemsQuotation().getBillTo());
		// doc.addField( "qEventDetails",
		// je.getJemsQuotation().getEventDetails());
		// }
		//
		// if(je.getJemsInvoice()!=null){
		// doc.addField( "iNumber",
		// je.getJemsInvoice().getINumber().toString());
		// doc.addField( "iDate", je.getJemsInvoice().getIDate());
		// doc.addField( "iDescription1",
		// je.getJemsInvoice().getDescription1());
		// doc.addField( "iDescription2",
		// je.getJemsInvoice().getDescription2());
		// doc.addField( "iDescription3",
		// je.getJemsInvoice().getDescription3());
		// doc.addField( "iDescription4",
		// je.getJemsInvoice().getDescription4());
		// doc.addField( "iDescription5",
		// je.getJemsInvoice().getDescription5());
		// doc.addField( "iBillTo", je.getJemsInvoice().getBillTo());
		// doc.addField( "iEventDetails",
		// je.getJemsInvoice().getEventDetails());
		// }
	...
	public String query(String q, boolean chkdates, boolean chkpaid,
		...
		try {
			...
			List<JemsOrganization> orgs = new ArrayList<JemsOrganization>(
			...
			if (chkdates && startdatetime != null && enddatetime != null) {
				k.append("AND startDateTime:[")
						.append(sdf.format(startdatetime)).append(" TO ")
						.append(sdf.format(enddatetime)).append("] ");
			}
			...
			if (chkeventstatus && eventstatus != null
					&& eventstatus.length() != 0) {
				k.append("AND status:" + eventstatus + " ");
			}

			log.info("SolrQuery built! user: " + user.getUserName()
					+ " query: " + k.toString());
			...
			log.info("Search successful. results: " + docs.getNumFound()
					+ " query: " + q);
			...
			results.put("success",
					"Search completed at " + df2.format(new Date())
							+ " - Elapsed Time: " + rsp.getElapsedTime() + "ms");
			results.put("error", "");

			return new JSONSerializer().exclude("*.class").deepSerialize(
					results);
			...
			results.put("query", query.getQuery());
			results.put("success", "");
			results.put("error", "Search Error at " + df2.format(new Date())
					+ " - " + e.getMessage());
