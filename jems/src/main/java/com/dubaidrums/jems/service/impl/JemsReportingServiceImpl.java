...
public class JemsReportingServiceImpl implements JemsReportingService {
	...
	public String generateQuotationPdf(JemsQuotation jq) throws Exception{
		...
		String reportPath = System.getProperty("java.io.tmpdir")+"/"+fileName;
		
		JasperReport jr = getReport("WEB-INF/reports/"+org.getId()+"/jemsQuotation.jasper"); 
		...
		log.info("method: generateQuotationPdf, msg: generated quotation pdf at "+reportPath);
	...
	private JasperReport getReport(String path) throws Exception{			
		...
		return (JasperReport) JRLoader.loadObjectFromFile(servletContext.getRealPath("/")+path);
	}
	...
	public String generateInvoicePdf(JemsInvoice jq) throws Exception{
		...
		JasperReport jr = getReport("WEB-INF/reports/"+org.getId()+"/jemsInvoice.jasper"); 
		JasperPrint print = JasperFillManager.fillReport(jr, parameters, new JREmptyDataSource());	
		JasperExportManager.exportReportToPdfFile(print, reportPath);
		
		log.info("method: generateInvoicePdf, msg: generated invoice pdf at "+reportPath);
	...
	private static String getTax(JemsTax[] taxes_, int i) {
		if(i<taxes_.length){
			return taxes_[i].getName()+" ("+formatNumber(taxes_[i].getRatePercent())+"%)";
		}
		else 
			return null;
	}
	...
	public String generateCostingsPdf(JemsEvent je) throws Exception {
		...
		List<JemsCostingItem> jcostings = jemsCostingItemService.findByJemsEventId(je.getId());
		...
		JasperReport jr = getReport("WEB-INF/reports/"+je.getOrganization().getId()+"/jemsCostings.jasper"); 
		JasperPrint print = JasperFillManager.fillReport(jr, p, new JREmptyDataSource());			
		JasperExportManager.exportReportToPdfFile(print, reportPath);		
		
		log.info("method: generateCostingsPdf, msg: generated costings pdf at "+reportPath);
...
//		}
		...
		JasperReport jr = getReport("WEB-INF/reports/"+je.getOrganization().getId()+"/jemsJobsheet2.jasper"); 
		JasperPrint print = JasperFillManager.fillReport(jr, p, new JREmptyDataSource());			
		JasperExportManager.exportReportToPdfFile(print, reportPath);		
		
		log.info("method: generateJobsheetPdf, msg: generated jobsheet pdf at "+reportPath);
