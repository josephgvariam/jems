...
public class JemsQuotationController {
	...
	public String generatePdf(@PathVariable("id") Long id,
		...
		try {
			log.info("user: " + principal.getName()
					+ ", method: generatePdf, quotationId: " + id
					+ ", msg: generating quotation pdf");
			String fileName = jemsReportingService.generateQuotationPdf(jq);

			jemsLogService.newLog(principal.getName()
					+ " generated a PDF quotation: <a href='/jemsquotations/"
					+ id + "'>" + jq.getQNumber()
					+ "</a> for event: <a href='/jemsevents/"
					+ jq.getJemsEvent().getId() + "'>"
					+ jq.getJemsEvent().getTitle() + "</a>");
			jemsEmailService.sendEmail("PDF Quotation Generated: "
					+ jq.getJemsEvent().getTitle(), principal.getName()
					+ " generated a PDF quotation: <a href='/jemsquotations/"
					+ id + "'>" + jq.getQNumber()
					+ "</a> for event: <a href='/jemsevents/"
					+ jq.getJemsEvent().getId() + "'>"
					+ jq.getJemsEvent().getTitle() + "</a>", fileName);

			return "redirect:/jemsquotations/pdf/"
	...
	public String create(@Valid JemsQuotation jemsQuotation,
		...
		jemsLogService.newLog(principal.getName()
				+ " created a new quotation: <a href='/jemsquotations/"
				+ jemsQuotation.getId() + "'>" + jemsQuotation.getQNumber()
				+ "</a> for event: <a href='/jemsevents/" + je.getId() + "'>"
				+ je.getTitle() + "</a>");
		jemsEmailService.sendEmail(
				"Quotation Created: " + je.getTitle(),
				principal.getName()
						+ " created a new quotation: <a href='/jemsquotations/"
						+ jemsQuotation.getId() + "'>"
						+ jemsQuotation.getQNumber()
						+ "</a> for event: <a href='/jemsevents/" + je.getId()
						+ "'>" + je.getTitle() + "</a>", null);
		log.info("user: " + principal.getName() + ", method: create, eventId: "
				+ je.getId() + ", quotationId: " + jemsQuotation.getId()
				+ ", quotationNumber: " + jemsQuotation.getQNumber()
				+ ", msg: new quotation created");

		return "redirect:/jemsquotations/"
				+ encodeUrlPathSegment(jemsQuotation.getId().toString(),
	...
	public String show(@PathVariable("id") Long id,
		...
		if (jq.isActive()) {
			uiModel.addAttribute(
					"previousJemsQuotations",
					jemsQuotationService.findByActiveAndJemsEvent(false,
							jq.getJemsEvent()));
		}
		...
		log.info("user: " + principal.getName() + ", method: show, eventId: "
				+ jq.getJemsEvent().getId() + ", quotationId: " + id
				+ ", quotationNumber: " + jq.getQNumber()
				+ ", msg: quotation showed");
	...
	// jemsQuotationService.findJemsQuotationEntries(firstResult, sizeNo));
	// float nrOfPages = (float) jemsQuotationService.countAllJemsQuotations() /
	// sizeNo;
	...
	public String update(@Valid JemsQuotation jemsQuotation,
			BindingResult bindingResult, Model uiModel,
			HttpServletRequest httpServletRequest, Principal principal) {
		if (bindingResult.hasErrors()) {
			populateEditForm(uiModel, jemsQuotation);
		...
		uiModel.asMap().clear();
		jemsQuotation.setModifiedUser(jemsUserService
				.findJemsUserByUserName(principal.getName()));
		jemsQuotation = jemsQuotationService.updateJemsQuotation(jemsQuotation);

		JemsEvent je = jemsQuotation.getJemsEvent();
		jemsLogService.newLog(principal.getName()
				+ " updated a quotation: <a href='/jemsquotations/"
				+ jemsQuotation.getId() + "'>" + jemsQuotation.getQNumber()
				+ "</a> for event: <a href='/jemsevents/" + je.getId() + "'>"
				+ je.getTitle() + "</a>");
		jemsEmailService.sendEmail("Quotation Updated: " + je.getTitle(),
				principal.getName()
						+ " updated a quotation: <a href='/jemsquotations/"
						+ jemsQuotation.getId() + "?jemsEventId=" + je.getId()
						+ "'>" + jemsQuotation.getQNumber()
						+ "</a> for event: <a href='/jemsevents/" + je.getId()
						+ "'>" + je.getTitle() + "</a>", null);
	...
	// }
	// return new ResponseEntity<String>(jemsQuotation.toJson(), headers,
	// HttpStatus.OK);
	// }
	//
	...
	// HttpHeaders headers = new HttpHeaders();
	// headers.add("Content-Type", "application/json; charset=utf-8");
	// List<JemsQuotation> result =
	// jemsQuotationService.findAllJemsQuotations();
	// return new ResponseEntity<String>(JemsQuotation.toJsonArray(result),
	...
	// JemsQuotation jemsQuotation =
	// JemsQuotation.fromJsonToJemsQuotation(json);
	...
	// json) {
	// for (JemsQuotation jemsQuotation:
	// JemsQuotation.fromJsonArrayToJemsQuotations(json)) {
	// jemsQuotationService.saveJemsQuotation(jemsQuotation);
	// }
	// HttpHeaders headers = new HttpHeaders();
	// headers.add("Content-Type", "application/json");
