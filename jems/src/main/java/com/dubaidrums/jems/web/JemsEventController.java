...
public class JemsEventController {
    ...
    public String create(@Valid JemsEvent jemsEvent, BindingResult bindingResult, Model uiModel, Principal principal, HttpServletRequest httpServletRequest) {
		JemsUser user = jemsUserService.findJemsUserByUserName(principal.getName());
		
        String recurringeventparams = httpServletRequest.getParameter("recurringeventparams");
		String recurringeventsummary = httpServletRequest.getParameter("recurringeventsummary");
		...
		if(jemsEvent.getEndDateTime().before(jemsEvent.getStartDateTime()) || jemsEvent.getEndDateTime().equals(jemsEvent.getStartDateTime())){
			bindingResult.rejectValue("endDateTime", "date.invalid");
		}
		
		if(recurringeventparams==null || recurringeventparams.length()==0){
			if(jemsEvent.getEndDateTime()!=null && jemsEvent.getStartDateTime()!=null && jemsEvent.getEndDateTime().before(jemsEvent.getStartDateTime())){
				bindingResult.rejectValue("endDateTime", "jemsevent.enddate.invalid");
			}
		}
        ...
        //RECURRING EVENTS
        if(recurringeventparams!=null && recurringeventparams.length()>0){
        	log.info("user: "+user.getUserName()+", method: create, msg: creating recurring event, recurringEventParams: "+recurringeventparams+", recurringEventSummary: "+recurringeventsummary);
        	jemsEventService.generateRepeatEvents(recurringeventparams, jemsEvent);
        	
        	jemsLogService.newLog(user.getUserName()+" created a new recurring event: "+jemsEvent.getTitle()+" - "+recurringeventsummary);
        	jemsEmailService.sendEmail("Repeat Event Created: "+jemsEvent.getTitle(), user.getUserName()+" created a new recurring event: "+jemsEvent.getTitle()+" - "+recurringeventsummary, null);
        ...
        jemsLogService.newLog(user.getUserName()+" created a new event: <a href='/jemsevents/"+jemsEvent.getId()+"'>"+jemsEvent.getTitle()+"</a>");
        jemsEmailService.sendEmail("Event Created: "+jemsEvent.getTitle(), user.getUserName()+" created a new event: <a href='/jemsevents/"+jemsEvent.getId()+"'>"+jemsEvent.getTitle()+"</a>", null);
        log.info("user: "+user.getUserName()+", method: create, eventId: "+jemsEvent.getId()+", msg: new event created");
        
        return "redirect:/jemsevents/" + encodeUrlPathSegment(jemsEvent.getId().toString(), httpServletRequest)+"?msg=1";
    }

	@PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_MANAGER','ROLE_OPERATOR')")
	@RequestMapping(params = "form", produces = "text/html")
    public String createForm(@RequestParam(value = "startDateTime", required = false) Date startDateTime, @RequestParam(value = "endDateTime", required = false) Date endDateTime, Model uiModel, Principal principal) {
		if(startDateTime==null) startDateTime = jemsUtilService.getCurrentDate();//new Date();
		if(endDateTime==null) endDateTime = jemsUtilService.getCurrentDate();//new Date();
		...
		JemsOrganization org = jemsOrganizationService.findJemsOrganization((long) 1);
    ...
    public String show(@PathVariable("id") Long id, @RequestParam(value = "msg", required = false) Integer msg, Model uiModel, Principal principal) {        
        ...
        log.info("user: "+principal.getName()+", method: show, eventId: "+id+", msg: event showed");
    ...
    public String duplicate(@PathVariable("id") Long id, Model uiModel, Principal principal) {    
		JemsEvent je = jemsEventService.findJemsEvent(id);

        
        
        log.info("user: "+principal.getName()+", method: duplicate, eventId: "+id+", msg: event duplicated");
        
        
        populateEditForm(uiModel, jemsEventService.buildCopy(je), jemsUserService.findJemsUserByUserName(principal.getName()));
        return "jemsevents/create";
    }	
	
	@RequestMapping(value = "/{id}/jobsheet", produces = "text/html")
    public String jobsheet(@PathVariable("id") Long id, Model uiModel, Principal principal) {   
        uiModel.addAttribute("jemsevent", jemsEventService.findJemsEvent(id));
        
        
        List<JemsCostingItem> jcostings = jemsCostingItemService.findByJemsEventId(id);
        ...
        uiModel.addAttribute("jemscostings", new JSONSerializer().exclude("*.class").exclude("jemsEvent").exclude("organization").exclude("rate").exclude("version").serialize(costings));
        
        log.info("user: "+principal.getName()+", method: jobsheet, eventId: "+id+", msg: jobsheet showed");
	...
	public String generateJobsheetPdf(@PathVariable("id") Long id, Model uiModel, Principal principal, HttpServletRequest httpServletRequest){
		JemsEvent je = jemsEventService.findJemsEvent(id);
		try {			
			log.info("user: "+principal.getName()+", method: generateJobsheetPdf, eventId: "+id+", msg: generating jobsheet pdf");
			String fileName = jemsReportingService.generateJobsheetPdf(je);
						
			return "redirect:/jemsevents/jobsheetpdf/" + encodeUrlPathSegment(fileName, httpServletRequest);
		} catch (Exception e) {
	...
	@ResponseBody
	public FileSystemResource getJobsheetPdfFile(@PathVariable("fileName") String fileName){	
	...
	public String generateCostingsPdf(@PathVariable("id") Long id, Model uiModel, Principal principal, HttpServletRequest httpServletRequest){
		JemsEvent je = jemsEventService.findJemsEvent(id);
		try {			
			log.info("user: "+principal.getName()+", method: generateCostingsPdf, eventId: "+id+", msg: generating costings pdf");
			String fileName = jemsReportingService.generateCostingsPdf(je);
						
			return "redirect:/jemsevents/costingspdf/" + encodeUrlPathSegment(fileName, httpServletRequest);
		} catch (Exception e) {
			log.error("Unable to generate PDF!", e);
			throw new RuntimeException("Unable to generate PDF!", e);
		}
	...
	@ResponseBody
	public FileSystemResource getCostingsPdfFile(@PathVariable("fileName") String fileName){	
		return new FileSystemResource(System.getProperty("java.io.tmpdir")+"/"+fileName+".pdf");		
	}	

	@RequestMapping(produces = "text/html")
    public String list(@RequestParam(value = "page", required = false) Integer page, Model uiModel, Principal principal) {
		JemsUser user = jemsUserService.findJemsUserByUserName(principal.getName());
		...
		Page<JemsEvent> p = jemsEventService.findJemsEventEntries(user, firstResult, sizeNo);
...
//        }
        ...
        log.info("user: "+principal.getName()+", method: list, page: "+page+", msg: events listed");
    ...
    public String update(@Valid JemsEvent jemsEvent, BindingResult bindingResult, Model uiModel, Principal principal, HttpServletRequest httpServletRequest) {
		JemsUser user = jemsUserService.findJemsUserByUserName(principal.getName());
...
//        JemsEvent oldJemsEvent = jemsEventService.findJemsEvent(jemsEvent.getId());
...
//        jemsEvent.setJemsInvoice(oldJemsEvent.getJemsInvoice());
        ...
        jemsLogService.newLog(user.getUserName()+" updated an event: <a href='/jemsevents/"+jemsEvent.getId()+"'>"+jemsEvent.getTitle()+"</a>");
        jemsEmailService.sendEmail("Event Updated: "+jemsEvent.getTitle(), user.getUserName()+" updated an event: <a href='/jemsevents/"+jemsEvent.getId()+"'>"+jemsEvent.getTitle()+"</a>", null);
        log.info("user: "+principal.getName()+", method: update, eventId: "+jemsEvent.getId()+", msg: event updated");
    ...
    public String delete(@PathVariable("id") Long id, Model uiModel, Principal principal) {
        JemsEvent jemsEvent = jemsEventService.findJemsEvent(id);
        jemsEventService.deleteJemsEvent(jemsEvent);
        uiModel.asMap().clear();
        
        jemsLogService.newLog(principal.getName()+" deleted an event: "+jemsEvent.getTitle());
        jemsEmailService.sendEmail("Event Deleted: "+jemsEvent.getTitle(), principal.getName()+" deleted an event: "+jemsEvent.getTitle(), null);
        log.info("user: "+principal.getName()+", method: delete, eventId: "+jemsEvent.getId()+", msg: event deleted");
	...
	void populateEditForm(Model uiModel, JemsEvent jemsEvent, JemsUser user) {
		...
		List<JemsCountry> jemsCountrys = jemsCountryService.findActiveJemsCountrys();
		List<JemsCurrency> jemsCurrencys = jemsCurrencyService.findActiveJemsCurrencys();
		List<JemsRegion> jemsRegions = jemsRegionService.findActiveJemsRegions();						        
        uiModel.addAttribute("jemscountrys", jemsCountrys);
        uiModel.addAttribute("jemscurrencys", jemsCurrencys);                
        uiModel.addAttribute("jemsusers", jemsUserService.findEnabledJemsUsers());
        uiModel.addAttribute("jemsstaff", jemsStaffService.findActiveJemsStaffs());
        uiModel.addAttribute("jemspaymentmethods", Arrays.asList(JemsPaymentMethod.values()));
        uiModel.addAttribute("jemseventstatuses", Arrays.asList(JemsEventStatus.values()));
        uiModel.addAttribute("jemseventtypes", Arrays.asList(JemsEventType.values()));        
        
        JemsOrganization org = jemsOrganizations.iterator().next();
        uiModel.addAttribute("jemsregions", jemsRegionService.findByCountry(org.getCountry()));        
        
        uiModel.addAttribute("jemsOrganizationsJSON", new JSONSerializer().exclude("*.class").serialize(jemsOrganizations));
        //uiModel.addAttribute("jemsCountrysJSON", new JSONSerializer().exclude("*.class").serialize(jemsCountrys));
        uiModel.addAttribute("jemsRegionsJSON", new JSONSerializer().exclude("*.class").serialize(jemsRegions));
                
        //addDateTimeFormatPatterns(uiModel);
        //uiModel.addAttribute("jemsinvoices", jemsInvoiceService.findAllJemsInvoices());
...
//        }
//        return new ResponseEntity<String>(jemsEvent.toJson(), headers, HttpStatus.OK);
//    }
//
...
//        return new ResponseEntity<String>(JemsEvent.toJsonArray(result), headers, HttpStatus.OK);
//    }
//
...
//    }
    ...
    @ResponseBody
    public ResponseEntity<String> getCalendarEventsListJson(@PathVariable("selecteddate") String selectedDate, Principal principal) {		
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        
        JemsUser user = jemsUserService.findJemsUserByUserName(principal.getName());
        List<JemsEvent> result = jemsEventService.getCalendarEventsList(selectedDate, user);
        ...
        for (JemsEvent je : result) {        	
			...
			if(je.getEndDateTime().before(je.getStartDateTime()) || je.getEndDateTime().equals(je.getStartDateTime())){
        ...
        log.info("user: "+principal.getName()+", method: getCalendarEventsListJson, selectedDate: "+selectedDate+", msg: returned "+events.size()+" events");
        
        return new ResponseEntity<String>(new JSONSerializer().exclude("*.class").serialize(events), headers, HttpStatus.OK);
    }
	
	@PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_MANAGER')")
	@RequestMapping(value = "/reports", headers = "Accept=application/json")
    @ResponseBody
    public ResponseEntity<String> getReportData(@RequestParam(value = "startDate", required = true) String startDate, @RequestParam(value = "endDate", required = true) String endDate, @RequestParam(value = "type", required = true) String type, @RequestParam(value = "org", required = true) Long org, Principal principal) {
		JemsUser user = jemsUserService.findJemsUserByUserName(principal.getName());
		Map<String, List> data = jemsEventService.getReportData(startDate, endDate, type, org, user);;		
		
        log.info("user: "+principal.getName()+", method: getReportData, msg: got report data for type: "+type+", org: "+org);
    ...
    public String getCostings(@PathVariable("id") Long id, @RequestParam(value = "msg", required = false) Integer msg, Model uiModel, Principal principal) {
		List<JemsCostingItem> costings = jemsCostingItemService.findByJemsEventId(id);
		
		uiModel.addAttribute("jemsEventId", id);
		uiModel.addAttribute("costings", new JSONSerializer().exclude("*.class").exclude("jemsEvent").exclude("organization").serialize(costings));
        
        log.info("user: "+principal.getName()+", method: getCostings, eventId: "+id+", msg: costings showed");
    ...
    public ResponseEntity<String> saveCostings(@PathVariable("id") Long id, @RequestBody String json, Principal principal) {
		JemsEvent jemsEvent = jemsEventService.findJemsEvent(id);
		List<JemsCostingItem> costingItems = new JSONDeserializer<List<JemsCostingItem>>().use(null, ArrayList.class).use("values", JemsCostingItem.class).deserialize(json);
		...
		jemsLogService.newLog(principal.getName()+" updated Costings for event: <a href='/jemsevents/"+jemsEvent.getId()+"'>"+jemsEvent.getTitle()+"</a>");
        log.info("user: "+principal.getName()+", method: saveCostings, eventId: "+id+", msg: costings saved");
    ...
    public ResponseEntity<String> getLogs(Principal principal) {
        ...
        log.info("user: "+principal.getName()+", method: getLogs, msg: logs showed");
        
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(new JSONSerializer().exclude("*.class").serialize(logs), headers, HttpStatus.OK);
    }	
	
	
//	@PreAuthorize("hasRole('ROLE_ADMIN')")
//	@RequestMapping(method = RequestMethod.GET, value = "/copycostings", produces = "text/html")
//	public ResponseEntity<String> copyCostings(Principal principal) {
//		List<JemsCostingCategory> categories = jemsCostingCategoryService.findAllJemsCostingCategorysByOrganizationId((long) 1);
//		List<JemsOrganization> orgs = jemsOrganizationService.findAllJemsOrganizations();
//		
...
//		HttpHeaders headers = new HttpHeaders();
//		headers.add("Content-Type", "application/json; charset=utf-8");
//		return new ResponseEntity<String>("Copy Costings", headers,HttpStatus.OK);
//	}
	
	
//	@PreAuthorize("hasRole('ROLE_ADMIN')")
//	@RequestMapping(method = RequestMethod.GET, value = "/fixgps", produces = "text/html")
...
//			if(e.getLocationLatLong().toLowerCase().indexOf("map")>-1 || e.getLocationLatLong().toLowerCase().indexOf("hybrid")>-1 || e.getLocationLatLong().toLowerCase().indexOf("satellite")>-1){
...
//	private Map<String, JemsClient> clientsMap = new HashMap<String, JemsClient>();
//	
//	@PreAuthorize("hasRole('ROLE_ADMIN')")
//	@RequestMapping(method = RequestMethod.GET, value = "/fixclient", produces = "text/html")
...
//        HttpHeaders headers = new HttpHeaders();
//        headers.add("Content-Type", "application/json; charset=utf-8");
//        return new ResponseEntity<String>("Fix Client: "+clientsMap.size(), headers, HttpStatus.OK);
//    }
//
...
//
//	@RequestMapping(method = RequestMethod.GET, value = "/fixalldocs", produces = "text/html")
...
//	
//	@RequestMapping(method = RequestMethod.GET, value = "/fixmissingdocs", produces = "text/html")
...
//		headers.add("Content-Type", "application/json; charset=utf-8");
//		System.out.println("========================== DONE ==================================");
//		return new ResponseEntity<String>("Fix Missing Docs. Quotations: "+missing_q+", Invoices: "+missing_i, headers, HttpStatus.OK);
//	}
//
//private List<Long> findEvent(JemsInvoice i) {
//	List<Long> result = new ArrayList<Long>();
//	for (JemsEvent e : events) {		
//		//if(jemsEventService.getBillTo(e).trim().equals(q.getBillTo().trim()) || jemsEventService.getEventDetails(e).trim().equals(q.getEventDetails().trim())){
//		if(compare(i,e)){
...
//		if(e.getUseInDocs()==0){
//			if(contactPerson.equals(e.getClientContactPerson().trim()) && company.equals(e.getClientCompany().trim())){
...
//			}
//		}else{
//			if(contactPerson.equals(e.getHiringAgentContactPerson().trim()) && company.equals(e.getHiringAgentCompany().trim())){
//				 //System.out.println("q: "+q.getEventDetails().trim()+" e: "+jemsEventService.getEventDetails(e).trim());
//				 return true;
//			}else {
//				result = false;
...
//		
//		String startDate2 = "Start Date: "+dateFormat.format(e.getStartDateTime());
...
//		
//		if(startDate.equals(startDate2) && endDate.equals(endDate2) && numDrums.equals(numDrums2) && type.equals(type2)){
//			return true;
//		}
//		
////		if( c(startDate,startDate2) && c(endDate,endDate2) && c(numDrums,numDrums2) && c(type,type2)){
////			return true;
////		}
//		
//	}
    ...
    public ResponseEntity<String> fixqi(Principal principal) {
		...
		for (JemsEvent e : events) {
			...
			for (JemsInvoice ji : invoices) {
				...
				if(ji.getJemsQuotation()==null && e.getJemsQuotations().size()!=0){
					log.info("event: "+e.getId()+", Invoive id: "+ji.getId()+", iN: "+ji.getINumber()+" No parent quotation.");
				}
			}
			if(icount>1){
