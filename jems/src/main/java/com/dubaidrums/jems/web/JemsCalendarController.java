...
public class JemsCalendarController {
	...
	public String showCalendar(Model uiModel, Principal principal) {
		...
		String json = new JSONSerializer().exclude("*.class").exclude("active")
				.exclude("adminEmail").exclude("country").exclude("currency")
				.exclude("defaultGps").exclude("region").exclude("version")
				.serialize(orgs);
