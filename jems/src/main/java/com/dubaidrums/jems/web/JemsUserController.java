...
public class JemsUserController {
	...
	public String create(@Valid JemsUser jemsUser, BindingResult bindingResult,
			Model uiModel, HttpServletRequest httpServletRequest,
			Principal principal) {
		if (jemsUserService.findJemsUserByUserNameIgnoreCase(jemsUser
				.getUserName()) != null) {
		...
		// encodeUrlPathSegment(jemsUser.getId().toString(),
		// httpServletRequest);
		log.info("user: " + principal.getName() + ", method: create, userId: "
				+ jemsUser.getId() + ", msg: new user created");
	...
	public String list(
		...
		if (page != null || size != null) {
			...
			uiModel.addAttribute("jemsusers",
					jemsUserService.findJemsUserEntries(firstResult, sizeNo));
			float nrOfPages = (float) jemsUserService.countAllJemsUsers()
					/ sizeNo;
			...
			uiModel.addAttribute("jemsusers",
					jemsUserService.findAllJemsUsers());
		}
		log.info("user: " + principal.getName() + ", method: list, page: "
				+ page + ", msg: users listed");
		return "jemsusers/list";
	}

	@RequestMapping(method = RequestMethod.PUT, produces = "text/html")
	public String update(@Valid JemsUser jemsUser, BindingResult bindingResult,
			Model uiModel, HttpServletRequest httpServletRequest,
			Principal principal) {
		JemsUser u = jemsUserService.findJemsUserByUserNameIgnoreCase(jemsUser
				.getUserName());
		...
		String oldEncryptedPasswd = jemsUserService.findJemsUser(
				jemsUser.getId()).getPassword();
		...
		// encodeUrlPathSegment(jemsUser.getId().toString(),
		// httpServletRequest);
		log.info("user: " + principal.getName() + ", method: update, userId: "
				+ jemsUser.getId() + ", msg: user updated");
		return "redirect:/jemsusers?msg=1";
	}
	...
	// }
	// return new ResponseEntity<String>(jemsUser.toJson(), headers,
	// HttpStatus.OK);
	// }
	//
	...
	// return new ResponseEntity<String>(JemsUser.toJsonArray(result), headers,
	// HttpStatus.OK);
	// }
	//
