...
public class JemsTaxController {
	...
	public String create(@Valid JemsTax jemsTax, BindingResult bindingResult,
		...
		// encodeUrlPathSegment(jemsTax.getId().toString(), httpServletRequest);
		log.info("user: " + principal.getName() + ", method: create, taxId: "
				+ jemsTax.getId() + ", msg: new tax created");
	...
	public String update(@Valid JemsTax jemsTax, BindingResult bindingResult,
			Model uiModel, HttpServletRequest httpServletRequest,
			Principal principal) {
		if (bindingResult.hasErrors()) {
			populateEditForm(uiModel, jemsTax);
	...
	// }
	// return new ResponseEntity<String>(jemsTax.toJson(), headers,
	// HttpStatus.OK);
	// }
	//
	...
	// return new ResponseEntity<String>(JemsTax.toJsonArray(result), headers,
	// HttpStatus.OK);
	// }
	//
