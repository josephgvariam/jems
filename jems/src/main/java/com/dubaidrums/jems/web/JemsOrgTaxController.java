...
public class JemsOrgTaxController {
    ...
    public String create(@Valid JemsOrgTax jemsOrgTax, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        ...
        //return "redirect:/jemsorgtaxes/" + encodeUrlPathSegment(jemsOrgTax.getId().toString(), httpServletRequest);
        log.info("user: "+principal.getName()+", method: create, taxId: "+jemsOrgTax.getId()+", msg: new tax created");
...
//    }
    ...
    public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "msg", required = false) Integer msg, Model uiModel, Principal principal) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("jemsorgtaxes", jemsOrgTaxService.findJemsOrgTaxEntries(firstResult, sizeNo));
            float nrOfPages = (float) jemsOrgTaxService.countAllJemsOrgTaxes() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
        	if(msg!=null && msg==1){
            	uiModel.addAttribute("msg", "Tax sucessfully saved!");
            }
            uiModel.addAttribute("jemsorgtaxes", jemsOrgTaxService.findAllJemsOrgTaxes());
        }
    ...
    public String update(@Valid JemsOrgTax jemsOrgTax, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, jemsOrgTax);
        ...
        jemsOrgTaxService.saveJemsOrgTax(jemsOrgTax);
        //return "redirect:/jemsorgtaxes/" + encodeUrlPathSegment(jemsOrgTax.getId().toString(), httpServletRequest);
        log.info("user: "+principal.getName()+", method: update, taxId: "+jemsOrgTax.getId()+", msg: tax updated");
        return "redirect:/jemsorgtaxes?msg=1";
    }
