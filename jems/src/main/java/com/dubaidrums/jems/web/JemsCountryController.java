...
public class JemsCountryController {
    ...
    public String create(@Valid JemsCountry jemsCountry, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        ...
        //return "redirect:/jemscountrys/" + encodeUrlPathSegment(jemsCountry.getId().toString(), httpServletRequest);
        log.info("user: "+principal.getName()+", method: create, countryId: "+jemsCountry.getId()+", msg: new country created");
...
//        uiModel.addAttribute("jemscountry", jemsCountryService.findJemsCountry(id));
...
//    }
    ...
    public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "msg", required = false) Integer msg, Model uiModel, Principal principal) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("jemscountrys", jemsCountryService.findJemsCountryEntries(firstResult, sizeNo));
            float nrOfPages = (float) jemsCountryService.countAllJemsCountrys() / sizeNo;
        ...
        log.info("user: "+principal.getName()+", method: list, page: "+page+", msg: countries listed");
        return "jemscountrys/list";
    }

	@RequestMapping(method = RequestMethod.PUT, produces = "text/html")
    public String update(@Valid JemsCountry jemsCountry, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, jemsCountry);
        ...
        //return "redirect:/jemscountrys/" + encodeUrlPathSegment(jemsCountry.getId().toString(), httpServletRequest);
        log.info("user: "+principal.getName()+", method: update, countryId: "+jemsCountry.getId()+", msg: country updated");
        return "redirect:/jemscountrys?msg=1";
    }
...
//        }
//        return new ResponseEntity<String>(jemsCountry.toJson(), headers, HttpStatus.OK);
//    }
//
...
//        return new ResponseEntity<String>(JemsCountry.toJsonArray(result), headers, HttpStatus.OK);
//    }
//
...
//        for (JemsCountry jemsCountry: JemsCountry.fromJsonArrayToJemsCountrys(json)) {
//            jemsCountryService.saveJemsCountry(jemsCountry);
//        }
//        HttpHeaders headers = new HttpHeaders();
//        headers.add("Content-Type", "application/json");
