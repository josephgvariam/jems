...
public class JemsCurrencyController {
	...
	public String create(@Valid JemsCurrency jemsCurrency,
		...
		// encodeUrlPathSegment(jemsCurrency.getId().toString(),
		// httpServletRequest);
		log.info("user: " + principal.getName()
				+ ", method: create, currencyId: " + jemsCurrency.getId()
				+ ", msg: new currency created");
	...
	// uiModel.addAttribute("jemscurrency",
	// jemsCurrencyService.findJemsCurrency(id));
	...
	public String list(
		...
		if (page != null || size != null) {
			...
			final int firstResult = page == null ? 0 : (page.intValue() - 1)
					* sizeNo;
			uiModel.addAttribute("jemscurrencys", jemsCurrencyService
					.findJemsCurrencyEntries(firstResult, sizeNo));
			float nrOfPages = (float) jemsCurrencyService
					.countAllJemsCurrencys() / sizeNo;
		...
		log.info("user: " + principal.getName() + ", method: list, page: "
				+ page + ", msg: currencies listed");
		return "jemscurrencys/list";
	}

	@RequestMapping(method = RequestMethod.PUT, produces = "text/html")
	public String update(@Valid JemsCurrency jemsCurrency,
			BindingResult bindingResult, Model uiModel,
			HttpServletRequest httpServletRequest, Principal principal) {
		if (bindingResult.hasErrors()) {
			populateEditForm(uiModel, jemsCurrency);
		...
		// httpServletRequest);
		log.info("user: " + principal.getName()
				+ ", method: update, currencyId: " + jemsCurrency.getId()
	...
	// }
	// return new ResponseEntity<String>(jemsCurrency.toJson(), headers,
	// HttpStatus.OK);
	// }
	//
	...
	// return new ResponseEntity<String>(JemsCurrency.toJsonArray(result),
	...
	// json) {
	// for (JemsCurrency jemsCurrency:
	// JemsCurrency.fromJsonArrayToJemsCurrencys(json)) {
	// jemsCurrencyService.saveJemsCurrency(jemsCurrency);
	// }
	// HttpHeaders headers = new HttpHeaders();
	// headers.add("Content-Type", "application/json");
