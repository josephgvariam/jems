...
public class JemsStaffController {
    ...
    public String create(@Valid JemsStaff jemsStaff, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        ...
        //return "redirect:/jemsstaffs/" + encodeUrlPathSegment(jemsStaff.getId().toString(), httpServletRequest);
        log.info("user: "+principal.getName()+", method: create, staffId: "+jemsStaff.getId()+", msg: new staff created");
...
//    }
    ...
    public String list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "msg", required = false) Integer msg, Model uiModel, Principal principal) {
        if (page != null || size != null) {
            ...
            uiModel.addAttribute("jemsstaffs", jemsStaffService.findJemsStaffEntries(firstResult, sizeNo));
            float nrOfPages = (float) jemsStaffService.countAllJemsStaffs() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
        	if(msg!=null && msg==1){
            	uiModel.addAttribute("msg", "Staff sucessfully saved!");
            }
            uiModel.addAttribute("jemsstaffs", jemsStaffService.findAllJemsStaffs());
        }
        log.info("user: "+principal.getName()+", method: list, page: "+page+", msg: staffs listed");
        return "jemsstaffs/list";
    }

	@RequestMapping(method = RequestMethod.PUT, produces = "text/html")
    public String update(@Valid JemsStaff jemsStaff, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, jemsStaff);
        ...
        //return "redirect:/jemsstaffs/" + encodeUrlPathSegment(jemsStaff.getId().toString(), httpServletRequest);
        log.info("user: "+principal.getName()+", method: update, staffId: "+jemsStaff.getId()+", msg: staff updated");
        return "redirect:/jemsstaffs?msg=1";
    }
