...
public class JemsInvoiceController {
	...
	public String generatePdf(@PathVariable("id") Long id, HttpServletRequest httpServletRequest, Principal principal){
		...
		try {			
			log.info("user: "+principal.getName()+", method: generatePdf, eventId: "+ji.getJemsEvent().getId()+", msg: generating invoice pdf");
			String fileName = jemsReportingService.generateInvoicePdf(ji);
			
			jemsLogService.newLog(principal.getName()+" generated a PDF invoice: <a href='/jemsinvoices/"+id+"'>"+ji.getINumber()+"</a> for event: <a href='/jemsevents/"+ji.getJemsEvent().getId()+"'>"+ji.getJemsEvent().getTitle()+"</a>");
			jemsEmailService.sendEmail("PDF Invoice Generated: "+ji.getJemsEvent().getTitle(), principal.getName()+" generated a PDF invoice: <a href='/jemsinvoices/"+id+"'>"+ji.getINumber()+"</a> for event: <a href='/jemsevents/"+ji.getJemsEvent().getId()+"'>"+ji.getJemsEvent().getTitle()+"</a>", fileName);
			
			return "redirect:/jemsinvoices/pdf/" + encodeUrlPathSegment(fileName, httpServletRequest);
		} catch (Exception e) {
    ...
    public String create(@Valid JemsInvoice jemsInvoice, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        ...
        jemsLogService.newLog(principal.getName()+" created a new invoice: <a href='/jemsinvoices/"+jemsInvoice.getId()+"'>"+jemsInvoice.getINumber()+"</a> for event: <a href='/jemsevents/"+jemsInvoice.getJemsEvent().getId()+"'>"+jemsInvoice.getJemsEvent().getTitle()+"</a>");
        jemsEmailService.sendEmail("Invoice Created: "+jemsInvoice.getJemsEvent().getTitle(), principal.getName()+" created a new invoice: <a href='/jemsinvoices/"+jemsInvoice.getId()+"'>"+jemsInvoice.getINumber()+"</a> for event: <a href='/jemsevents/"+jemsInvoice.getJemsEvent().getId()+"'>"+jemsInvoice.getJemsEvent().getTitle()+"</a>", null);
        log.info("user: "+principal.getName()+", method: create, eventId: "+jemsInvoice.getJemsEvent().getId()+", invoiceId: "+jemsInvoice.getId()+", invoiceNumber: "+jemsInvoice.getINumber()+", msg: new invoice created");
        
        return "redirect:/jemsinvoices/" + encodeUrlPathSegment(jemsInvoice.getId().toString(), httpServletRequest)+"?msg=1";
    }

	@PreAuthorize("hasAnyRole('ROLE_ADMIN','ROLE_MANAGER','ROLE_OPERATOR')")
	@RequestMapping(params = "form", produces = "text/html")
    public String createForm(@RequestParam(value="jemsEventId", required = true) Long jemsEventId, @RequestParam(value="jemsQuotationId", required = false) Long jemsQuotationId, Model uiModel) {
    ...
    public String show(@PathVariable("id") Long id, @RequestParam(value = "msg", required = false) Integer msg, Model uiModel, Principal principal) {
        ...
        if(ji.isActive()){
        	uiModel.addAttribute("previousJemsInvoices", jemsInvoiceService.findByActiveAndJemsEvent(false, ji.getJemsEvent()));
        }
        ...
        log.info("user: "+principal.getName()+", method: show, eventId: "+ji.getJemsEvent().getId()+", invoiceId: "+id+", invoiceNumber: "+ji.getINumber()+", msg: invoice showed");
...
//            uiModel.addAttribute("jemsinvoices", jemsInvoiceService.findJemsInvoiceEntries(firstResult, sizeNo));
//            float nrOfPages = (float) jemsInvoiceService.countAllJemsInvoices() / sizeNo;
...
//    }
    ...
    public String update(@Valid JemsInvoice jemsInvoice, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest, Principal principal) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, jemsInvoice);
            return "jemsinvoices/update";
        }
        uiModel.asMap().clear();
        jemsInvoice.setModifiedUser(jemsUserService.findJemsUserByUserName(principal.getName()));
        jemsInvoice = jemsInvoiceService.updateJemsInvoice(jemsInvoice);
        

        jemsLogService.newLog(principal.getName()+" updated an invoice: <a href='/jemsinvoices/"+jemsInvoice.getId()+"'>"+jemsInvoice.getINumber()+"</a> for event: <a href='/jemsevents/"+jemsInvoice.getJemsEvent().getId()+"'>"+jemsInvoice.getJemsEvent().getTitle()+"</a>");
        jemsEmailService.sendEmail("Invoice Updated: "+jemsInvoice.getJemsEvent().getTitle(), principal.getName()+" updated an invoice: <a href='/jemsinvoices/"+jemsInvoice.getId()+"'>"+jemsInvoice.getINumber()+"</a> for event: <a href='/jemsevents/"+jemsInvoice.getJemsEvent().getId()+"'>"+jemsInvoice.getJemsEvent().getTitle()+"</a>", null);
        log.info("user: "+principal.getName()+", method: update, eventId: "+jemsInvoice.getJemsEvent().getId()+", invoiceId: "+jemsInvoice.getId()+", invoiceNumber: "+jemsInvoice.getINumber()+", msg: invoice updated");
        
        //uiModel.addAttribute("jemsEventId", jemsEventId);
        uiModel.addAttribute("msg", 1);
        return "redirect:/jemsinvoices/" + encodeUrlPathSegment(jemsInvoice.getId().toString(), httpServletRequest);
    }
...
//        }
//        return new ResponseEntity<String>(jemsInvoice.toJson(), headers, HttpStatus.OK);
//    }
//
...
//        return new ResponseEntity<String>(JemsInvoice.toJsonArray(result), headers, HttpStatus.OK);
//    }
//
...
//        for (JemsInvoice jemsInvoice: JemsInvoice.fromJsonArrayToJemsInvoices(json)) {
//            jemsInvoiceService.saveJemsInvoice(jemsInvoice);
//        }
//        HttpHeaders headers = new HttpHeaders();
//        headers.add("Content-Type", "application/json");
