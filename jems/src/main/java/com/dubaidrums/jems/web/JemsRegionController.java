...
public class JemsRegionController {
	...
	public String create(@Valid JemsRegion jemsRegion,
		...
		// encodeUrlPathSegment(jemsRegion.getId().toString(),
		// httpServletRequest);
		log.info("user: " + principal.getName()
				+ ", method: create, regionId: " + jemsRegion.getId()
				+ ", msg: new region created");
	...
	public String list(
		...
		if (page != null || size != null) {
			...
			final int firstResult = page == null ? 0 : (page.intValue() - 1)
					* sizeNo;
			uiModel.addAttribute("jemsregions", jemsRegionService
					.findJemsRegionEntries(firstResult, sizeNo));
			float nrOfPages = (float) jemsRegionService.countAllJemsRegions()
					/ sizeNo;
		...
		log.info("user: " + principal.getName() + ", method: list, page: "
				+ page + ", msg: regions listed");

		return "jemsregions/list";
	}

	@RequestMapping(method = RequestMethod.PUT, produces = "text/html")
	public String update(@Valid JemsRegion jemsRegion,
			BindingResult bindingResult, Model uiModel,
			HttpServletRequest httpServletRequest, Principal principal) {
		if (bindingResult.hasErrors()) {
			populateEditForm(uiModel, jemsRegion);
		...
		// httpServletRequest);
		log.info("user: " + principal.getName()
				+ ", method: update, regionId: " + jemsRegion.getId()
				+ ", msg: region updated");
		return "redirect:/jemsregions?msg=1";
	}
	...
	// }
	// return new ResponseEntity<String>(jemsRegion.toJson(), headers,
	// HttpStatus.OK);
	// }
	//
	...
	// return new ResponseEntity<String>(JemsRegion.toJsonArray(result),
