...
//		System.out.println("eventstatus:"+eventstatus);
        ...
        String results = jemsSearchService.query(q,
        		chkdates,
        		chkpaid,
        		chktype,
        		chkeventstatus,
    ...
    public ResponseEntity<String> createIndex(Principal principal) {
        log.info("user: "+principal.getName()+", method: createIndex, msg: creating search index");

        jemsSearchService.createIndex(jemsEventService.findAllJemsEvents());
        
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>("Create Index", headers, HttpStatus.OK);
    }
	
	@PreAuthorize("hasRole('ROLE_ADMIN')")
	@RequestMapping(value = "/deleteindex", produces = "text/html")
    public ResponseEntity<String> deleteIndex(Principal principal) {
        log.info("user: "+principal.getName()+", method: deleteIndex, msg: deleting search index");
        
        jemsSearchService.deleteIndex();
        
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>("Delete Index", headers, HttpStatus.OK);
    }		
